...
class RegistrationForm(FlaskForm):
    ...
    confirm_pswd = PasswordField('confirm_pswd_label', validators=[InputRequired(message="Password required"), Length(min=4,max=25,message="Password must be between 4 and 25 characters"), EqualTo('password' , message = 'Passwords must match')])
    submit_button = SubmitField('Create')

    def validate_username(self , username):
        user_object = User.query.filtery_by(username = username.data).first()
        if user_object:
